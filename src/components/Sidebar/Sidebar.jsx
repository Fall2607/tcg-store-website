...
function Sidebar({ isExpanded, setIsExpanded }) {
  ...
  return (
    <div className={`fixed top-0 left-0 h-screen bg-neutral-600 text-white transition-all duration-300 z-50 
      ...
      <div>
        ...
        <nav className="mt-4 flex flex-col gap-2 px-2 text-sm">
          {[
            ...
            { to: "/order-items", icon: assets.orderItemIcon, label: "Order Items" },
          ].map(({ to, icon, label }) => (
            <NavLink
              key={to}
              to={to}
